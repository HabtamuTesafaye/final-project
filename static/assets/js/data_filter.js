...
function getTranslation(word) {
  ...
  // Construct payload with an economic focus
  const payload = {
      contents: [
          {
              parts: [
                  {
                      text: `Provide a detailed economic description of the term: ${word} in one paragraph. Focus on its significance, impact, or role in economics.`
                  }
              ]
          }
      ]
...
.then(data => {
  console.log(data); // Log the response data to see the structure
  ...
  if (candidates.length > 0) {
      const text = candidates[0]?.content?.parts[0]?.text || 'No description available.';
      descriptionElement.innerText = text;
  } else {
      descriptionElement.innerText = 'No description returned from API.';
  }
})
.catch(error => {
    console.error('Error fetching description:', error);
    descriptionElement.innerText = 'Failed to load description. Please try again later.';
});
}
...
document.getElementById('toggleButton').addEventListener('click', () => {
  ...
  const languagePair = document.getElementById('languageSelector').value;

  // Toggle between Translate and Revert mode
  if (toggleButton.innerText === 'Translate') {
      let textToTranslate = descriptionElement.innerText.trim();
      if (!textToTranslate) return;

      descriptionElement.setAttribute('data-original-text', textToTranslate); // Save the original text
      descriptionElement.innerText = 'Translating...';
      ...
      const textChunks = splitText(textToTranslate, 500);
      let translatedChunks = [];

      // Function to translate each chunk sequentially
      function translateChunks(index = 0) {
          if (index >= textChunks.length) {
              // Combine translated chunks and display
              descriptionElement.innerText = translatedChunks.join(' ');
              toggleButton.innerText = 'Revert to English'; // Change button to Revert mode
              return;
          }

          const apiUrl = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(textChunks[index])}&langpair=${languagePair}`;

          fetch(apiUrl)
              .then(res => res.json())
              .then(data => {
                  translatedChunks.push(data.responseData.translatedText);
                  translateChunks(index + 1); // Translate the next chunk
              })
              .catch(error => {
                  console.error('Error during translation:', error);
                  descriptionElement.innerText = 'Failed to translate. Please try again.';
              });
      }
  ...
  } else if (toggleButton.innerText === 'Revert to English') {
      // Revert to original text
      const originalText = descriptionElement.getAttribute('data-original-text');
      if (originalText) {
          descriptionElement.innerText = originalText;
      }
      toggleButton.innerText = 'Translate'; // Change button back to Translate mode
  }
});
                    ...
                    $("#button").on("click", "#applyButton", function () {
                        ...
                        table += `
                            <tr>
                                <td class="border" style="width: auto; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    <div class="row">
                                        <div class="col-10">
                                            <p style="font-size: small; white-space: nowrap;" class="d-block fw-bold text-dark">
                                                ${title_ENG} ${title_amharic} 
                                                <span class="measurement-text" style="color: red;">${measure}</span>
                                                <i class="bi bi-info-circle ms-2" style="cursor: pointer; color: #28a745;" data-bs-toggle="modal" data-bs-target="#configsite" onclick="getTranslation('${title_ENG}')"></i>
                                            </p>
                                        </div>
                                    </div>
                                </td>`;
                                  ...
                                  <tr>
                                      <td class="border bg-white" style="width: auto; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                          <div class="d-flex align-items-center">
                                              <a>
                                                  <h6 class="mb-1">
                                                      <p style="font-size: small;" class="d-block text-dark fw-normal m-0">
                                                          &nbsp;&nbsp; ${indicator.title_ENG}
                                                      </p>
                                                  </h6>
                                              </a>
                                               <i class="bi bi-info-circle ms-2" style="cursor: pointer; color: #28a745;" data-bs-toggle="modal" data-bs-target="#configsite" onclick="getTranslation('${indicator.title_ENG}')"></i>
                                          </div>
                                      </td>`;
                      ...
                      let typeMonthTable = () => {
                        ...
                        for (filterIndicator of filterIndicators) {
                          ...
                          // modal for information
                          table += `
                          <th class="vertical-text border">
                              <p class="fw-bold text-dark p-0 m-0">
                                  ${filterIndicator.title_ENG} ${title_amharic}
                                  <span class="measurement-text" style="color: red;">${measure}</span>
                                   <i class="bi bi-info-circle ms-2" style="cursor: pointer; color: #28a745;" data-bs-toggle="modal" data-bs-target="#configsite" onclick="getTranslation('${filterIndicator.title_ENG}')"></i>
                              </p>
                          </th>`;
                          ...
                          for (let indicator of data.indicators) {
                            if (
                              ...
                              test = true;
                              table += `
                              <th class="vertical-text fw-normal border">
                                  &nbsp;&nbsp; ${indicator.title_ENG}
                                   <i class="bi bi-info-circle ms-2" style="cursor: pointer; color: #28a745;" data-bs-toggle="modal" data-bs-target="#configsite" onclick="getTranslation('${title_ENG}')"></i>
                              </th>`;
